...
function startTimer() {
    ...
    const startDateStr = startTime.toLocaleString();

    startTimeEl.textContent = startTimeStr;
    recordStartTimeEl.textContent = startTimeStr;
    document.getElementsById("recordStartDate").textContent = 
    startDateStr;
    ...
    timerInterval = setInterval(()=>{
        ...
        elapsedTimeEl.textContent = timeFormatting(elapsedTime);
    },1000);
...
function stopTimer() {
    ...
    const endDateStr = endTime.toLocaleString();

    const startTimeStr = startTime.toLocaleString();
    const startDateStr = startTime.toLocaleString();

    endTimeEl.textContent = endTimeStr;
    recordEndTimeEl.textContent = endDateStr;
...
function stopTimer() {
  ...
  const totalDurationStr = timeFormatting(totalSeconds);
  taskData.totalDuration = totalDurationStr;

  elapsedTimeEl.textContent = durationStr;
  endTimeEl.textContent = session.endTime;
  recordEndTimeEl.textContent = session.endTime;
  recordEndDateEl.textContent = session.endDate;
  taskDurationEl.textContent = durationStr;
